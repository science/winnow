...
topicsMore / topicsLess: SUBJECT tags only (lowercase) they want more/less of — things a video can be about, like "chess" or "rook endgames". At most 8 per list; no near-synonym variants ("engineering" already covers "engineering practice") — but qualified tags are NOT synonyms of each other, never drop one to save space. When they qualify a subject by tier, level, or style, keep it as a two-word tag "<qualifier> <subject>" using exactly one of these qualifier words: ${DIGEST_TIER_QUALIFIERS.join(", ")} — and do NOT emit the bare parent tag: "chess videos featuring top tier play" → topicsMore "elite chess" — never bare "chess" and never free-form qualifiers like "top tier chess". When their wording spans two qualifiers, emit both: "low tier comic chess games" → topicsLess "comedic chess" AND "amateur chess". Videos are tagged with these exact qualifier words, so synonyms will not match. Emit the bare subject only when their interest genuinely spans all of it. Quality complaints are NOT topics: "clickbait", "hype", "overclaiming", "science provocateurs" and similar describe how a video is made, not what it is about — they belong in the numeric axes above (claimOverreach, clickbaitSeverity), never in topicsLess.
