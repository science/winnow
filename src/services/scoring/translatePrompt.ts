...
export const TRANSLATOR_PROMPT_VERSION = 5;
...
You may also receive their recent personal votes on videos ("up" = good pick, "down" = not for me). Some votes include the video's analyzed digest — its topic tags (with tier qualifiers), format, tone, and scores on the axes above. Generalize in those coordinates: repeated downs sharing a tier-qualified topic strengthen that topicsLess tag; downs with low substanceDensity raise that axis's importance; ups show which constraints matter most. Votes without a digest carry only title/channel — weigh them more cautiously. Never add a constraint their profile text and votes don't support.`;
