...
- **RANKER_VERSION 3 — qualified topics survive matching (2026-07-15, the gotham mis-ranking):** a low-tier comic chess video ranked top with the contradictory reason "on-profile: chess; avoided topic: chess". The translator had preserved the user's qualifiers ("comic chess" avoided, "top tier play" sought) but bidirectional-substring `topicMatch` collapsed them: avoid item "comic chess" fired on every digest tagged "chess", the seek/avoid hits cancelled in the weighted average, and quality axes floated the video up. Now: a profile item matches a digest topic iff every item token appears in that single topic (order/plural-insensitive, no union across tags); an avoid-topic hit caps the score at 45 (just under Worth-a-look — formats/tones stay uncapped as a feed-collapse guard) and leads the reason, which names the profile item, not the digest topic. `canonicalizeTarget` drops token-superset synonym spam. Translator prompt v3 keeps tier/level/style qualifiers and forbids emitting the bare parent tag for a restricted interest. Qualifier wording is a shared controlled vocabulary (`DIGEST_TIER_QUALIFIERS`: elite/professional/amateur/beginner/comedic/casual) interpolated into BOTH prompts — free-choice qualifiers never token-match across two independent LLM calls ("top tier chess" vs "elite chess"). On the digest side the tier is a schema-forced multi-select (`subjectTiers`, since a low-elo blunder recap is comedic AND amateur), folded into qualified leading topics by `clampDigest` — nano reliably answers a forced enum but not a free-text composition rule (verified empirically: it emitted "beginner tutorial" for a comedic blunder recap under the free-text rule). End-to-end synthetic repro: a comedic low-elo recap transcript now ranks 44 ("avoided: comedic chess") vs elite content ≥75. Repro fixture: `src/services/youtube/fixtures/gotham-poor-ranking-capture.json.gz` (scrubbed via `scripts/scrub-capture.ts`) + `src/lib/fixtures/gotham-target.json`.

- **Enrich v3 + translator v5 — celebrity-crossover tiers and digest-bearing votes (2026-07-21, the Tyler1/Faker mis-ranking):** "TYLER1 vs FAKER INSANE CHESS BATTLE" (famous streamer + LoL pro in a casual exhibition) got subjectTier `elite` — the enrich prompt's "top players → elite" had no guard against fame in another domain, and the folded "elite chess" tag matched the seek list while dodging every avoid tag. Enrichment prompt v3: tier means skill *demonstrated in this video's subject*; fame elsewhere never makes play elite; crossover/celebrity exhibitions → casual (+comedic/+amateur as shown). Translator prompt v5 closes the exact-match register hole: rejecting a whole register ("low tier comic chess") now emits every covered qualifier (comedic AND amateur AND casual chess) — one missing variant is a hole in the filter. Votes also snapshot the video's digest now (`FeedbackEntry.digest`/`digestPromptVersion`, captured in `toggleVote` from the enrichment cache) and ride into the translation as compact digest coordinates (`recentExamples` includes them only when the snapshot matches the current enrichment prompt version — an older prompt's digest may carry the very mislabel the newer prompt fixed). Live specs: crossover-tier + register-span cases in `e2e/live/twoPhase.spec.ts`; probe any live video via `scripts/probe-video.ts <id> --title=... --fresh-target` (also fixed: it was silently probing without transcripts — wrong outcome key).
