...
| `src/services/scoring/twoPhase.ts` | Two-phase orchestrator: enrichment cache, transcript pass, translation cache, ranking; fixed cheap models (`claude-haiku-4-5`/`gpt-5.4-nano`); `expectedScoresHash` gates optimistic display of stored scores. Diagnose ranking/collapse issues at scale with `npx vite-node scripts/twophase-diag.ts 100 openai` (live provider calls, real videos) | Two-phase pipeline |
