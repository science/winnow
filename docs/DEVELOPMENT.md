...
| `src/services/youtube/subscribe.ts` | **The only write to the user's YouTube account.** `subscribeToChannel` (signed InnerTube POST, never throws, per-stage failure markers, no retry) + `subConfirmationUrl` fallback. Reuses DNR rule 2 — do NOT add a rule | Subscribe flow; read the header before touching |
| `src/services/youtube/authCookies.ts` | `getSapisid()` — the sole reader of the `cookies` permission; null outside extension contexts | SAPISID/auth issues |
| `src/lib/sapisidHash.ts` | `sapisidHashHeader` (SHA-1 over `<ts> <SAPISID> <origin>`), known-vector tested | Rarely |
| `src/lib/subscriptions.ts` | Pure `proxySubscribedChannels` (degraded fallback set) + `subscribedIdSet` | Subscribed-set semantics |
| `src/stores/subscriptionsStore.ts` | Account-level subscribed channels (`winnow:subscriptions:v1`, TTL 30 min), `subscribedIds` derived, `refreshSubscriptions(videos)` (takes videos as an arg — importing feedStore would be a cycle), `addSubscribedChannel` | Subscription state |
...
- **`cookies` exists for exactly one call.** `authCookies.getSapisid` is its only reader and `subscribe.ts` its only caller. Widening that is a privacy-policy and AMO-listing change (`PRIVACY.md`, `docs/store/AMO_LISTING.md` item 3), not just a code change.
