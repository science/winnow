...
| `src/services/youtube/feedSource.ts` | `isDemoMode()`, demo fixtures, `loadFeeds()` merge/dedupe/cap (300), `loadSubscribedChannels()` (degrades to the subscriptions-feed proxy set), partial-failure warnings | Feed sources, demo behavior |
...
| `src/components/` | `App` (config gate + routes), `Feed` (tier sections, orphan player, route→player subscription), `FeedItem` (card + its inline player), `VideoCard`, `InlinePlayer` (nocookie embed, start-on-open, no autoplay-next), `ScoreBadge`, `Settings`, `Onboarding` | UI |
