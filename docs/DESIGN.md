...
- **Read-only against the YouTube account, and no cookie access.** Every YouTube call Winnow makes is a GET of a page the user could open themselves; nothing is posted, changed, or deleted. The extension requests no `cookies` permission — credentialed reads work because host permissions let `fetch(..., {credentials: "include"})` carry the browser's own session, and the extension never sees a cookie value. A signed subscribe write (SAPISIDHASH via the `cookies` permission) was built and then pulled back out before the 0.2.3 release, pending UAT; it lives in git history (added in `e7312a5`, removed in the 0.2.2 release prep) and is restorable with a revert. Any future write is a privacy-policy, AMO-listing, and permission-prompt change — treat re-adding `cookies` as a product decision, never a refactor.
